...
public class UIManager : Singleton<UIManager>
{
    ...
    {
        if ((panelStats.activeSelf==false)) //si el panel no está activo no hacemos nana
        {
            return;
        }
        //panel activo, recogemos los Stats

        statDañoTMP.text=stats.Daño.ToString();
        statDefensaTMP.text=stats.Defensa.ToString();
        statCriticoTMP.text=$"{stats.PorcentajeCritico}%"; //mostramos un porcentaje con lo que usamos string interpolation
    ...
    public void AbrirCrerarPanelInspectorQuests()
    {
    ...
    public void AbrirPanelInteraccion(InteraccionExtraNPC tipoInteraccion)
    {
        switch(tipoInteraccion)
        {
            case InteraccionExtraNPC.Quests:
                AbrirCrerarPanelInspectorQuests();
                break;
